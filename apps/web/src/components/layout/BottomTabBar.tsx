...
import { Home, Compass, Ticket, Sparkles, Crown, LogIn } from 'lucide-react'
      ...
      <nav className="floating-tab-bar pointer-events-auto flex h-[56px] w-full max-w-[360px] items-stretch px-1.5">
        ...
        {isAuthenticated ? (
          ...
          <button
            ...
            className="relative flex flex-1 flex-col items-center justify-center gap-0.5 py-1"
          >
            <LogIn className="relative z-10 h-5 w-5 text-[#0a84ff]" strokeWidth={2} />
            <span className="relative z-10 text-[10px] font-medium text-[#0a84ff]">Sign in</span>
          </button>
        )}
      </nav>
    </div>
