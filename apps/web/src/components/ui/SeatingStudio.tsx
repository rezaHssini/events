...
function FloorCanvas({
  ...
  const floor = (
    <div className="min-h-[320px] rounded-2xl border border-white/10 bg-gradient-to-b from-[#1a1a24] to-[#101018] p-4 shadow-inner lg:min-h-[480px] lg:p-6">
      {children}
    </div>
  )
                ...
                {section.seats
                  ...
                  .map((seat) => (
                    <button
                      ...
                      className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-md border text-[9px] font-semibold transition-transform hover:scale-105 lg:h-8 lg:w-8 lg:text-[10px] ${
...
function RestaurantFloorPlan({
  ...
  return (
    <div className="flex flex-col items-center py-2">
      ...
      <div className={`relative aspect-square w-full max-w-[240px] lg:max-w-[320px] ${table.blocked ? 'opacity-40' : ''}`}>
        {positions.map((pos, i) => {
...
function TheaterFloorOverview({
  ...
  return (
    <div className="space-y-3 py-1 lg:grid lg:grid-cols-2 lg:gap-3 lg:space-y-0">
...
function StatChip({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex-1 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-center">
...
type RightPanel = 'venue' | 'edit' | 'templates' | null
...
export function SeatingStudio({
  ...
  const applyTemplate = (templateId: string) => {
    ...
    if (isDesktop) setRightPanel('edit')
  ...
  const openVenueSettings = () => {
    if (isDesktop) setRightPanel('venue')
    else setVenueSheetOpen(true)
  }
  ...
  const openTemplates = () => {
    if (isDesktop) setRightPanel('templates')
    else setTemplateSheetOpen(true)
  }
  ...
  const fieldClass =
    'mt-1.5 w-full rounded-xl border border-white/10 bg-white/[0.06] px-3 py-2.5 text-sm outline-none focus:border-[#0a84ff]/50'

  const venuePanelBody = (
    <div className="space-y-4 text-left">
      ...
      <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3">
        <div>
          ...
          <p className="text-xs text-slate-500">Show stage on floor plan</p>
        </div>
        <button
          type="button"
          onClick={() =>
      ...
      {plan.stage.enabled && (
        <div>
          <p className="mb-2 text-xs text-slate-500">Stage position</p>
          <StagePositionPicker
      ...
      {plan.floors.length > 1 && (
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#ff453a]/15 py-3 text-sm font-medium text-[#ff453a]"
        >
    ...
    <>
      {activeFloor.mode === 'theater' && activeSection && (
        <div className="space-y-4 text-left">
          ...
          <div>
            <p className="mb-2 text-xs text-slate-500">Section color</p>
      ...
      {templates.map((template) => {
        ...
        return (
          <button
            ...
            className="flex w-full items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-left transition-colors hover:bg-white/[0.06]"
          >
            <div>
  ...
  const floorControls = (
    <div className="space-y-3">
      <div>
        <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-slate-500">Floors</p>
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-semibold text-white/70"
          >
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-full bg-white/8 px-2.5 py-1.5 text-xs font-medium text-white/55"
            >
              <Plus className="h-3.5 w-3.5" />
            </button>
          </div>
      ...
      {viewMode === 'detail' && activeFloor.mode === 'dining' && (
        <div>
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-slate-500">Tables</p>
          <div className="flex flex-wrap gap-1.5">
      ...
      <div className="mt-4 flex flex-wrap gap-2 lg:mt-6">
        {viewMode === 'overview' ? (
          <button
            ...
            className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] py-3 text-sm font-semibold hover:bg-white/[0.07]"
          >
        ...
        <button
          type="button"
          onClick={() =>
            activeFloor.mode === 'theater' ? addSection(activeFloor.id) : addTable(activeFloor.id)
          }
          className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#0a84ff]/20 py-3 text-sm font-semibold text-[#64b5ff] hover:bg-[#0a84ff]/30"
        >
  ...
  const content = (
    <motion.div
      ...
      exit={{ opacity: 0, y: 12 }}
      className="fixed inset-0 z-[80] flex flex-col bg-[#0a0a0f] lg:bg-[#08080c]"
    >
      <header className="shrink-0 border-b border-white/10 px-4 pb-3 pt-[max(env(safe-area-inset-top,0px),12px)] lg:px-8 lg:pb-4 lg:pt-5">
        <div className="flex items-center gap-2 lg:gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] hover:bg-white/[0.08] lg:h-10 lg:w-10"
            aria-label="Back"
          >
            <ChevronLeft className="h-5 w-5" />
          </button>
          <div className="min-w-0 flex-1 text-left">
            <p className="text-base font-semibold lg:text-lg">Seating studio</p>
            <p className="truncate text-xs text-slate-500 lg:text-sm">{activeFloor.name}</p>
          </div>
          <button
            type="button"
            onClick={openTemplates}
            className={`flex h-9 w-9 items-center justify-center rounded-full border hover:bg-white/[0.08] lg:h-10 lg:w-10 ${
              rightPanel === 'templates' && isDesktop
                ? 'border-[#0a84ff]/50 bg-[#0a84ff]/15'
                : 'border-white/10 bg-white/[0.04]'
            }`}
          ...
          <button
            type="button"
            onClick={handleSave}
            className="rounded-full bg-[#0a84ff] px-4 py-2 text-sm font-semibold text-white hover:bg-[#0077ed] lg:px-5"
          >
            Save
          </button>
        </div>

        <div className="mt-3 flex gap-2 lg:mt-4 lg:max-w-xl">
      ...
      <div className="flex min-h-0 flex-1">
        {isDesktop ? (
          <aside className="flex w-72 shrink-0 flex-col overflow-y-auto border-r border-white/10 p-5">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          {!isDesktop ? (
            <div className="sticky top-0 z-10 border-b border-white/10 bg-[#0a0a0f]/95 backdrop-blur-xl">
              <div className="space-y-2 px-4 py-3">{floorControls}</div>
            </div>
          ) : null}

          <div className="flex-1 overflow-y-auto px-4 py-4 lg:px-8 lg:py-6">
            <div className="mx-auto w-full max-w-lg lg:max-w-none">{canvasBlock}</div>
          </div>
        </div>

        {isDesktop ? (
          <aside className="flex w-80 shrink-0 flex-col border-l border-white/10">
            <div className="flex items-center gap-2 border-b border-white/10 px-5 py-4">
              <button
                type="button"
                onClick={() => setRightPanel('edit')}
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${
                  rightPanel === 'edit' ? 'bg-[#0a84ff] text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                {activeFloor.mode === 'theater' ? 'Section' : 'Table'}
              </button>
              <button
                type="button"
                onClick={() => setRightPanel('venue')}
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${
                  rightPanel === 'venue' ? 'bg-[#0a84ff] text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                Venue
              </button>
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${
                  rightPanel === 'templates' ? 'bg-[#0a84ff] text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                Templates
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              ...
              {rightPanel === 'venue' && venuePanelBody}
              {rightPanel === 'edit' && editPanelBody}
              {rightPanel === 'templates' && templatesPanelBody}
            </div>
          </aside>
        ) : null}
      </div>
