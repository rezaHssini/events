...
export default function PlannerPage() {
  ...
  return (
    <div className="min-h-screen">
      <header className="border-b border-white/10 bg-gradient-to-br from-warning/10 to-primary/10 px-4 py-5">
      ...
      <div className="p-4">
        ...
        <h2 className="mt-6 mb-3 text-left text-sm font-semibold">Recent events</h2>
        {managedEvents.slice(0, 3).map((event) => (
          <Link
            key={event.id}
            to={`/club/events/${event.id}`}
            className="mb-2 flex items-center gap-3 rounded-xl glass p-3 card-hover"
          >
            <img src={event.cover} alt="" className="h-12 w-12 rounded-lg object-cover" />
            <div className="flex-1 text-left">
