...
export function SeatingStudio({
  ...
  const content = (
    ...
    >
      ...
      <div className="shrink-0 border-t border-white/10 bg-[#0a0a0f]/95 px-4 py-3 pb-[max(env(safe-area-inset-bottom,0px),12px)]">
        <div className="mx-auto flex max-w-lg gap-2">
          {viewMode === 'overview' ? (
            <button
              ...
              className="flex flex-1 items-center justify-center gap-2 rounded-[16px] bg-[#0a84ff] py-3.5 text-sm font-semibold text-white"
            >
          ...
          <button
            ...
            className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[16px] liquid-glass-subtle"
            aria-label={activeFloor.mode === 'theater' ? 'Add section' : 'Add table'}
          >
            <Plus className="h-5 w-5 text-[#64b5ff]" />
          </button>
          {viewMode === 'detail' && (
            <button
              type="button"
        ...
        <p className="mx-auto mt-2 max-w-lg text-center text-xs text-white/40">
          {viewMode === 'overview'
            ? activeFloor.mode === 'theater'
