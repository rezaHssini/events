...
function ProfileContent({ profile, isOwn }: { profile: UserProfile; isOwn: boolean }) {
  ...
  const isTabRoot = isOwn && location.pathname === '/profile'
    ...
    <div className="min-h-screen bg-[#0a0a0f] pb-10">
      ...
      <div className="relative h-44 overflow-hidden">
        ...
        {isOwn && (
          <Link
            to="/settings"
            className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-black/50 backdrop-blur"
          >
      ...
      <div className="relative px-4">
        ...
        {isOwn && (
          <div className="mt-4">
            <BecomePlannerCard variant="profile" />
          </div>
        )}
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0f]">
        <header className="flex items-center gap-3 border-b border-white/10 px-4 py-4">
          <Link to={`/user/${handle}`} className="text-slate-400">
            <ChevronLeft className="h-5 w-5" />
          </Link>
          <h1 className="font-bold capitalize">{list}</h1>
        </header>
