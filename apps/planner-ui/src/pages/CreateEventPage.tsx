...
export default function CreateEventPage() {
      ...
      <div className="flex min-h-screen flex-col bg-[#0a0a0f]">
        ...
        <div className="border-t border-white/10 bg-black/80 p-4 backdrop-blur-xl">
          <div className="mx-auto flex w-full max-w-sm flex-col gap-3">
            <Link to="/" className="ios-button w-full py-3.5 text-center text-[17px]">
              Back to My Club
            </Link>
            <button
              type="button"
  ...
  return (
    <div className="min-h-screen pb-48">
      <MobileHeader title="Create Event" back="/" />
      ...
      {/* Footer actions */}
      <div className="fixed bottom-0 left-0 right-0 mx-auto max-w-2xl border-t border-white/10 bg-black/80 p-4 backdrop-blur-xl">
        <div className="flex gap-3">
          {step > 0 && (
            <button
              onClick={() => setStep(step - 1)}
              className="rounded-xl glass px-5 py-3 font-medium"
            >
              Back
            </button>
          )}
          {step < STEPS.length - 1 ? (
            <>
              ...
              <button
                onClick={() => setStep(step + 1)}
                className="flex-1 rounded-xl gradient-hero py-3 font-bold text-white"
              >
                Continue
              </button>
            </>
